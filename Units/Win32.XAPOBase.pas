...
const
    ...
    XAPOBASE_DEFAULT_FORMAT_TAG = WAVE_FORMAT_IEEE_FLOAT;
    // 32-bit float only, applies to WAVEFORMATEX.wFormatTag or WAVEFORMATEXTENSIBLE.SubFormat when used
    ...
    XAPOBASE_DEFAULT_FORMAT_BITSPERSAMPLE = 32;
    // 32-bit float only, applies to WAVEFORMATEX.wBitsPerSample and WAVEFORMATEXTENSIBLE.wValidBitsPerSample when used

    // default XAPO property flags supported, applies to XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS
    XAPOBASE_DEFAULT_FLAG = (XAPO_FLAG_CHANNELS_MUST_MATCH or XAPO_FLAG_FRAMERATE_MUST_MATCH or XAPO_FLAG_BITSPERSAMPLE_MUST_MATCH or
        XAPO_FLAG_BUFFERCOUNT_MUST_MATCH or XAPO_FLAG_INPLACE_SUPPORTED);
